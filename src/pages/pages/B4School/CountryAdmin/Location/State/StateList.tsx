...
class StateList extends Component<{}, StateList> {
  constructor(props: {}) {
    super(props);
    this.state = {
      rows: [],
  ...
  handleDeleteClick = (params: GridCellParams) => {
    ...
    this.setState({ selectedStateId: params.row.id })
  ...
  render() {
    ...
    const { rows, totalRows, loading, pageSize, page, openDelete, openView, openEdit, selectedCountryPage, selectedStateId } = this.state;
    ...
    return (
      <Card >
        ...
        {openDelete ? <DeleteState selectedStateId={selectedStateId} show={openDelete} handleclose={() => this.setState({ openDelete: false }, this.fetchData())} /> : null}


        {openEdit ? <EditState selectedCountryPage={selectedCountryPage} show={openEdit} handleclose={() => this.setState({ openEdit: false }, this.fetchData())} /> : null}
