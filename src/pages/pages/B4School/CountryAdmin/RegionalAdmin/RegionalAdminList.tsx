...
import EditRegionalAdmin from './EditRegionalAdmin';
import DeleteRegionalAdmin from './DeleteRegionalAdmin';
...
interface RegionalAdminListState {
  rows: staticPage[];
  totalRows: number;
  loading: boolean;
  pageSize: number;
...
class RegionalAdminList extends Component<{}, RegionalAdminListState> {
  constructor(props: {}) {
    super(props);
    this.state = {
      rows: [],
  ...
  fetchData = () => {
    ...
    axiosInstance.get(`/admin/v1/regionalAdmin/getAll?pageNo=${page}&limit=${pageSize}`)
      .then((response) => {
        this.setState({
          rows: response.data.data.regionalAdminData ? response.data.data.regionalAdminData : [],
  ...
  render() {
    ...
    return (
      <Card >
        <CardHeader title='Regional Admin' />
        ...
        {openDelete ? <DeleteRegionalAdmin selectedCountryId={selectedCountryId} show={openDelete} handleclose={() => this.setState({ openDelete: false }, this.fetchData())} /> : null}


        {openEdit ? <EditRegionalAdmin selectedCountryPage={selectedCountryPage} show={openEdit} handleclose={() => this.setState({ openEdit: false }, this.fetchData())} /> : null}
