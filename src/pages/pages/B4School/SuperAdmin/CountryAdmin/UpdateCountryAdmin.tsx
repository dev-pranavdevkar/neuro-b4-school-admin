...
export default function UpdateCountryAdmin({ show, handleclose, selectedCountryPage }) {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      const CountryAdmin = await axiosInstance.post(
        `/admin/v1/countryAdmin/updateById/${id}`,
        { ...data }
      ).then((response) => {
        setLoading(false)
  ...
  useEffect(() => {
    ...
    setValue('country_code', selectedCountryPage['country_code'])
    // setValue('password', selectedCountryPage['password'])
    // setValue('confirm_password', selectedCountryPage['confirm_password'])
