...
class AdmissionList extends Component<{}, AdmissionList> {
  constructor(props: {}) {
    super(props);
    this.state = {
      rows: [],
  ...
  fetchData = () => {
    ...
    axiosInstance.get(`/admin/v1/admissionForm/getAll?pageNo=${page}&limit=${pageSize}`)
      .then((response) => {
        this.setState({
          rows: response.data.data.AdmissionFormData ? response.data.data.AdmissionFormData : [],
  ...
  render() {
    ...
    return (
      <Card >
        <CardHeader title='Admissions Forms' />
